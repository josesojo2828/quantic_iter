...
async function main() {
  ...
  const role = await prisma.role.findUnique({ where: { slug: 'mentor_owner' } });

  if (!user || !tenant || !role) {
    console.error('❌ Error: No se encontró el usuario, el mentoría o el rol de dueño.');
  ...
  console.log(`🔗 Link creado: Usuario [${user.email}] <-> Academia [${tenant.name}] con Rol [${role.name}]`);
}
